...
Assign next task number (or insert at specified position, renumbering subsequent tasks).
...
```bash
mkdir -p .research/tasks/task-[NN]
```

## 5. Commit

```bash
git add .research/WORKFLOW.md .research/tasks/
git commit -m "research: add task [NN] — [task name]"
```
