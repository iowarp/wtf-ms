...
for record in .planning/state.json .planning/decisions.json .planning/structure/outline.json .planning/sources .planning/evidence; do
  [ ! -e "$record" ] || printf 'EXISTING: wtf-p 0.6 record %s\n' "$record"
done
...
C="${CLAUDE_CONFIG_DIR:-}"
[ -n "${C//[[:space:]]/}" ] || C="$HOME/.claude"
case "$C" in '~') C="$HOME" ;; '~/'*) C="$HOME/${C:2}" ;; esac
for R in "$C" ".claude"; do
  P="$R/marketplaces/wtfp/.claude-plugin/plugin.json"
  if [ -f "$P" ]; then
    printf 'MODERN (%s): inspect JSON %s\n' "$R" "$P"
    cat "$P"
...
K="${CLIO_CODER_CONFIG_DIR:-}"
[ -n "${K//[[:space:]]/}" ] || K="$HOME/.config/clio-coder"
case "$K" in '~') K="$HOME" ;; '~/'*) K="$HOME/${K:2}" ;; esac
...
Use read on each plugin.json and parse its JSON name/version; report malformed
JSON, missing fields, unexpected name or unreadable files as uncertain. Record
all modern/legacy/missing-command results for both roots, plus the Clio manifest
result. File presence establishes source only: report "source present, activation
unverified". Missing/incomplete modern sources still exclude the legacy writer.
Treat an existing Clio manifest as modern or uncertain, never as legacy-only.
...
- **LEGACY only, no MODERN or Clio manifest anywhere, no uncertainty, and no 0.6 records in `.planning/`** → Route: PROJECT.md (Step 8).
...
- **Nothing found** → Route: handoff (it is plain text and stays valid). Suggest, without running it:
  ...
  ```
- **CLIO present** → route to Step 7 and report "source present, activation unverified". For a new project, paste 01 into `/wtfp:new-paper` inside Clio Coder. For existing 0.6 records, paste the inspection variant into `/wtfp:progress` first. Then paste 02 and 03 in order, retaining each gate.
...
Write `.planning/config.json` only if it does not already exist, using default balanced settings and `venue_template` from the venue. Preserve every existing config.json; inspect it and report the preserved file rather than replacing it.

Use ls and read to verify every saved handoff file, or the legacy PROJECT.md
and config.json. Confirm saved content matches the author-approved preview before
reporting success. On the legacy route distinguish a preserved existing config
from a file this command actually wrote.

## 9. Record

Only if `commit_research` is true. Stage exactly what this command wrote, never a directory:
```bash
if grep -q '"commit_research": *true' .research/config.json 2>/dev/null; then
  if [ -n "$(git diff --cached --name-only)" ]; then
    echo "Optional record blocked: preserve existing staged changes."
  else
    git add .research/handoff/README.md .research/handoff/01-new-paper.md .research/handoff/02-map-project.md .research/handoff/03-create-outline.md && git commit -m "docs: paper handoff from wtfMS research — [paper title]"
  fi
fi
```
On the 0.5 route, check commit_research with the same bash grep gate and stage `.planning/PROJECT.md` and `.planning/config.json` by name instead, including only files this command actually wrote. Never stage `.planning/` as a directory.
...
wtf-p:   [0.6 source, activation unverified | 0.5 legacy | uncertain | not installed]
...
New project: `/wtfp:new-paper` ← paste 01-new-paper.md
Existing 0.6: `/wtfp:progress` ← paste 01 inspection instead
`/wtfp:map-project` ← paste 02-map-project.md
`/wtfp:create-outline` ← paste 03-create-outline.md

<sub>Paste each block into Clio and complete its gate before the next.</sub>
